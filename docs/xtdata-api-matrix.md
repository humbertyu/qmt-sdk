...
#### 重构后回归：2026-09-02（compat 与生产文件同日对照）

本轮在 bridge 重启后重新执行，所有结果写入 `D:\\Temp\\xtquant-regression-*`，不覆盖生产数据。
先进行单票/10 只验证，再进行全市场分批验证。`download_history_data2` 的返回值为 `{}`，
数据通过随后调用 `get_market_data_ex` 读取。

| 周期 | 股票数 | 批大小 | 下载/查询/保存耗时 | 返回行数 | 结果 |
|---|---:|---:|---:|---:|---|
| 1d | 5217 | 1000 | 总 420.287s；保存 18.226s | 5217 | 全部返回 |
| 1m | 5217 | 1000 | 总 638.154s；保存 20.339s | 1,257,297 | 全部返回 |
| tick | 5217 | 50 | 总 1595.870s；保存 172.928s | 20,550,556 | 5215 只返回数据，2 只无数据 |

1d 与生产 `data/market/cn/stock/snapshot/1d/{symbol}.parquet` 的 `20260902` 行级对照：
5217/5217 股票均存在，行数和日期一致；OHLC 仅 9 只出现约 `2.3e-13` 的机器浮点尾差，
volume 全部一致；amount 有 5143 只存在差异，最大绝对差约 51，属于金额字段的整数级
取整/计算差异，需按容差比较，不能视为时间或股票错位。

单票 `000779.SZ` 验证耗时：1d `download 0.258s/query 0.122s`、1m `0.207s/0.186s`、
tick `0.462s/0.201s`；10 只批量总耗时分别为 1d `1.20s`、1m `1.38s`、tick `2.85s`。
大批量耗时主要发生在 QMT 的历史下载阶段，而不是 parquet 保存或查询阶段。

#### instruments 同日逐字段对照（2026-09-02）

compat 结果保存于 `D:\\Temp\\xtquant-regression-instruments-20260902-compat.parquet`，
与生产 `data/reference/cn/stock/instrument_daily/2026.parquet` 中 `date=20260902` 的
5217 行进行 symbol 对齐。股票集合、行数和以下字段均一致：`is_st`、`pre_close`、
`settlement_price`、`up_limit`、`down_limit`、`float_shares`、`total_shares`、
`last_volume`。`status` 仅 2 条 compat 为空而生产有值。

`is_trading` 存在系统性差异：compat 为 True 5208/False 9，生产为 True 2/False 5215，
共有 5206 只股票不一致。该差异不能归因于股票集合或行情字段缺失；需要核对生产文件
生成时使用的交易状态判定时点/业务逻辑后，才能判定 instruments 迁移结果。

#### 1m 同日逐股票文件对照（2026-09-02）

compat 临时目录中的 5217 个文件与生产 1m 文件逐一匹配，按生产文件的 `date` 列筛选
`20260902` 后比较。5217/5217 股票均匹配，缺失 0，行数差异 0，时间索引差异 0。
`volume` 5217 只全部一致；OHLC 仅 9 只出现约 `4.55e-13` 的浮点尾差；`amount` 仅
1 只存在差异，最大绝对差约 1。结论：1m 的结构、时间索引和核心数值与生产同日数据一致，
仅保留机器浮点尾差及单条 amount 舍入差异。

#### 请求队列与订阅控制（2026-09-02）

bridge 保持 QMT `ContextInfo` 单线程执行模型，不在策略线程之外并发调用 QMT API，
也不改变 `download_history_data2` 等同步接口的阻塞语义。本次仅增加轻量调度规则：
外部请求可以并发提交，但 QMT 端仍一次处理一个完整请求；订阅/取消订阅等控制请求
在等待队列中优先于普通历史数据请求。行情事件继续使用独立事件通道，不进入普通请求队列。

该改动不改变任何返回结构或字段；需要专项回归“已有订阅 + 长查询 + 取消订阅”场景，
但不要求重跑已经完成的全量数据一致性比较。

专项回归结果：订阅建立耗时 `0.109s`，长任务期间收到 1 次事件；1000 只 1d 下载耗时
`28.483s`，期间提交的 `unsubscribe_quote` 等待 `26.699s` 后成功执行，随后查询耗时
`3.859s`。这证明控制请求可优先处理排队中的普通请求，但不会中断已经开始执行的 QMT
调用；取消会在当前调用结束后执行。

#### 重构后官方查询接口连通性探针（2026-09-02）

使用 `tools/probe_qmt_native_api.py` 通过 `qmt_sdk.QmtClient` 执行官方查询接口样本，
报告保存于 `D:\\Temp\\xtquant-regression-api-20260902.json`。除已知依赖 pandas 的
`get_turnover_rate` 外，其余 30 个查询接口样本均返回 `ok`，包括行情、历史数据、合约、
ETF、期权、除权、港股通、交易日历和 BSM 计算接口。下载/订阅/公式类接口按探针设计跳过，
不将“跳过”误记为失败；`get_raw_financial_data` 仍按约定跳过。
字段集合，后续日期仍需抽样复验。
